...
public static partial class WordBatchEmitter
{
        ...
        List<BatchItem> items, BodyEmitContext ctx)
    {
        ...
        {
            // BUG-DUMP-TOCOPENER-EXTREL: the typed fallback recreates the
            // hyperlink relationship but CANNOT reconstruct a cross-paragraph
            // field wrapper — it emits no field begin/instrText/separate. That is
            // fine for a non-opener member (a cached TOC entry; the wrapper rides
            // on the opener member), but when the bail lands on the OPENER itself
            // (the paragraph carrying fldCharType="begin") the whole field wrapper
            // is lost and the later members' raw-passed end fldChar is orphaned —
            // an unbalanced, malformed field. So for the opener, prefer to raw-pass
            // the paragraph verbatim (keeping the field wrapper) after stripping the
            // offending external hyperlink r:id (an absolute file/URL link out of a
            // TOC entry); the field structure is what matters, the dangling link
            // target is an acceptable loss. Fall back to the typed bail only when
            // an external ref remains (e.g. an external IMAGE r:embed) that this
            // strip can't neutralize without dropping content.
            bool isFieldOpener = rawP.Contains("w:fldCharType=\"begin\"", StringComparison.Ordinal);
            string strippedP = isFieldOpener ? StripHyperlinkExternalRels(rawP) : rawP;
            if (isFieldOpener && !HasExternalRelRef(strippedP))
            {
                ctx.Warnings.Add(new DocxUnsupportedWarning(
                    Element: "field.member",
                    Path: child.Path,
                    Reason: "cross-paragraph field opener carries a hyperlink with an external relationship; the field wrapper is preserved by raw round-trip but the external hyperlink target (r:id) is dropped to avoid a dangling relationship"));
                rawP = strippedP;
            }
            else
            {
